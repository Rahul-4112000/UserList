...
const UserList = memo(({ usersList, onEdit, onDeleteUser, onOpenEmptyUserForm }) => {
      ...
      return (
        <tr key={serialNo} align='center' className='p-4'>
          ...
          <td className='flex gap-4 justify-center py-2'>
            <button className='cursor-pointer' onClick={() => onEdit(user)}>
              <svg
  ...
  return (
    <div className='flex flex-col w-[1000px] m-auto mt-[30px]'>
      <Button btnType='success' btnName='Add User' styles='self-end' onClick={onOpenEmptyUserForm}></Button>
      <table className=''>
        <thead>
          <tr>
